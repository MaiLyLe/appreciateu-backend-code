...
class MessageStatisticsView(viewsets.ModelViewSet):
    ...
    def get_total_numbers_statistics_pie_chart(self, request):
        """Returns a dict of all necessary data for pie chart that displays the distribution between
        students and professors in sent and received messages for logged-in user"""
    ...
    def get_total_amount_array_bar_charts(self, request):
        """Returns dict of all necessary data for bar chart displaying total amount statistics
        for messages for logged-in user"""
        user = request.user
        ...
        current_month = (timezone.now() - timedelta(
            days=days_in_current_month)).replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        messages_received_6_months.append({"time": current_month,
                                           "count": this_month_messages_received})
        ...
        for x in range(0, 6):
            ...
            if x == 0:
                ...
                day_threshold_max = timezone.now()\
                    .replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=x)
            ...
            count_received_per_day = Message.objects.filter(
                ...
                timestamp__lt=day_threshold_max,
                receiver=user).count()
            messages_sent_7_days.append({"time": day_threshold_min,
                                         "count": count_sent_per_day})
            messages_received_7_days.append({"time": day_threshold_min,
                                             "count": count_received_per_day})
            ...
            count_received_per_month = Message.objects.filter(
                ...
                timestamp__lt=month_threshold_max,
                receiver=user).count()
            messages_sent_6_months.append({"time": month_threshold_min,
                                           "count": count_sent_per_month})
            messages_received_6_months.append({"time": month_threshold_min,
                                               "count": count_received_per_month})
